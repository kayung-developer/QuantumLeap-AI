...
const StrategyCard = ({ strategy, onClone, onSubscribe, isMutating, ownedByUser }) => {
    ...
    return (
        // --- 2. WRAP THE CARD CONTENT IN A Link ---
        // This makes the entire card clickable, leading to the public detail page.
        // We use `relative` and `flex` to ensure layout is maintained.
            ...
            <Card className="flex flex-col justify-between overflow-hidden w-full transition-transform transform hover:-translate-y-1 hover:shadow-xl">
                ...
                <div>
                    <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center text-sm text-light-muted dark:text-light-gray ml-2 flex-shrink-0"><FaUsers className="mr-1.5"/>{strategy.clone_count}</div>
                    </div>
                    ...
                    {perf && (
                        <div className="grid grid-cols-3 gap-2 text-center bg-light-main dark:bg-primary p-2 rounded-lg border border-light-border dark:border-border-color">
                            <div>
                ...
                <div
                    className="mt-4 z-20"
                    // Stop the Link navigation when clicking on the button itself
