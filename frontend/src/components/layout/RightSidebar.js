...
const NotificationItem = ({ notification }) => {
    ...
    return (
        <div className="p-3 bg-light-main dark:bg-primary rounded-md flex items-start space-x-3">
            ...
            <div>
                <p className="text-sm text-light-text dark:text-light-gray">{notification.message}</p>
...
const ActivityItem = ({ bot }) => {
    ...
    return (
        <Link to={`/dashboard/bots/${bot.id}`} className="p-3 bg-light-main dark:bg-primary rounded-md flex items-center space-x-3 hover:bg-light-hover dark:hover:bg-border-color transition-colors">
            <div className="flex-shrink-0">{statusIcon}</div>
            <div className="flex-grow min-w-0">
                <p className="text-sm font-semibold text-light-heading dark:text-white truncate">{bot.name}</p>
                <p className="text-xs text-light-muted truncate">{bot.strategy_name.replace(/_/g, ' ')} on {bot.symbol}</p>
            </div>
        </Link>
    );
};
...
const WalletItem = ({ wallet }) => (
    <div className="p-3 bg-light-main dark:bg-primary rounded-md flex items-center justify-between">
        <div className="flex items-center">
            {/* In a real app, you'd have an icon mapping */}
            <FaWallet className="mr-3 text-accent" />
            <div>
                <p className="font-bold text-light-heading dark:text-white">{wallet.asset}</p>
                <p className="text-xs text-light-muted">Balance</p>
            </div>
        </div>
        <p className="font-mono text-light-text dark:text-light-gray">{parseFloat(wallet.balance).toFixed(6)}</p>
    </div>
);
