...
const BotDetailPage = () => {
    ...
    const filterCondition = useMemo(() => (msg) => msg.bot_id === botId && ['bot_log', 'error', 'trade_executed'].includes(msg.type), [botId]);
    const realTimeLogs = useWebSocketListener(filterCondition);
    ...
    const getLogColor = (type) => {
        ...
        if (type === 'trade_executed') return 'text-success';
        ...
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-1 flex flex-col">
                     <h2 className="text-xl font-semibold text-white mb-4 flex items-center"><FaServer className="mr-3 text-accent" />Real-time Logs</h2>
                    <div className="h-96 bg-primary p-3 rounded-md overflow-y-auto flex flex-col-reverse flex-grow font-mono text-xs">
                        <AnimatePresence>
                            {realTimeLogs.map((log, index) => (
                                <motion.p
                                    ...
                                    exit={{ opacity: 0 }}
                                    className={`py-1 border-b border-border-color/50 ${getLogColor(log.type)}`}
                                >
                                   <span className="text-gray-500 mr-2">{new Date().toLocaleTimeString()}</span>
                                   {log.details ? `TRADE: ${log.details.side.toUpperCase()} ${log.details.amount.toFixed(5)} ${bot?.symbol.split('/')[0]} @ $${log.details.price.toFixed(2)}` : log.message}
                                </motion.p>
                            ))}
                        </AnimatePresence>
                    </div>
