...
class SerialNumber(Base):
    ...
    # --- NEW: Direct link to a user account ---
    assigned_user_id = Column(String, ForeignKey("users.id"), nullable=False, index=True)

    # --- NEW: The plan this serial key is valid for ---
    associated_plan = Column(String(10), nullable=False)

    is_active = Column(Boolean, default=True)

    # --- NEW: Enhanced activation tracking ---
    machine_id_hash = Column(String, nullable=True, unique=True)
    last_ip_hash = Column(String, nullable=True)  # Hash of the last known IP

    activated_at = Column(DateTime(timezone=True), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Relationship to easily fetch the user who this key belongs to
...
class TradingBotCreate(BaseModel):
    ...
    # --- NEW: Allow backtest_results_cache to be accepted (and ignored/parsed) ---
    backtest_results_cache: Optional[Any] = None
    ...
    # --- FIX: Validator to handle JSON strings if frontend sends them ---
...
class TradingBotSchema(BaseModel):
    ...
    @field_validator('strategy_params', 'visual_strategy_json', 'backtest_results_cache', mode='before')
    @classmethod
    def parse_json_strings(cls, v: Any) -> Any: # Return type Any
        if isinstance(v, str):
            if not v: return None
            try:
                return json.loads(v)
...
class PublicStrategySchema(BaseModel):
    ...
    @classmethod
    def parse_backtest_cache(cls, v: Any) -> Any: # Return type Any
        if isinstance(v, str):
            try:
                return json.loads(v)
            except json.JSONDecodeError:
...
# --- NEW: Pydantic model for generating a serial ---
class SerialNumberGenerateRequest(BaseModel):
    user_id: str # The ID of the user to assign this key to
    plan: SubscriptionPlan # The plan this key is for (e.g., "premium", "ultimate")

# --- Update SerialNumberActivateRequest (no changes needed, but good to verify) ---
class SerialNumberActivateRequest(BaseModel):
    serial_key: str
    machine_id: str

class SerialNumberActivateResponse(BaseModel):
    message: str
    activation_token: str # A short-lived JWT for the final login step

class ActivationTokenLoginRequest(BaseModel):
...
class EmaCrossAtrParams(BaseStrategyParams):
    # Updated to match frontend: short_window / long_window
    short_window: int = Field(50, gt=1);
    long_window: int = Field(200, gt=1);
    atr_period: int = Field(14, gt=1); # Default if not sent
    atr_multiplier: float = Field(1.5, gt=0.1); # Default if not sent
...
class EmaCrossAtrStrategy(AbstractStrategy):
    ...
    def generate_signal(self) -> TradingSignal:
        # Use pandas ewm for live calculation
        df = self.ohlcv
        p = self.parameters
        # Map frontend params to internal logic
        short_win = p.get('short_window', 50)
        long_win = p.get('long_window', 200)
        
        df['ema_fast'] = df['close'].ewm(span=short_win, adjust=False).mean()
        df['ema_slow'] = df['close'].ewm(span=long_win, adjust=False).mean()
    ...
    def _generate_signals_vectorized(df: pd.DataFrame, p: dict) -> pd.DataFrame:
        ...
        df_out['ema_fast'] = df_out['close'].ewm(span=short_win, adjust=False).mean()
        df_out['ema_long'] = df_out['close'].ewm(span=long_win, adjust=False).mean()
        ...
        df_out['reason'] = np.where(crossover, "EMA Crossover", np.where(crossunder, "EMA Crossunder", ""))
        return df_out
...
STRATEGY_REGISTRY = {
    "MA_Cross": EmaCrossAtrStrategy,
    "Bollinger_Bands": RsiBbMeanReversionStrategy, # Mapping BB to RSI/BB Reversion
    "RSI_MACD_Crossover": MacdAdxTrendStrategy, # Mapping generic trend to MACD/ADX
    "Volatility_Squeeze": VolatilitySqueezeStrategy,
    "SuperTrend_ADX_Filter": SuperTrendAdxStrategy,
    "Ichimoku_Cloud_Breakout": IchimokuBreakoutStrategy,
    "Smart_Money_Concepts": SmcOrderBlockFvgStrategy,
    "AI_Signal_Confirmation": AiEnhancedSignalStrategy,
    "Optimizer_Portfolio": OptimizerPortfolioStrategy,
    # Keep legacy keys just in case
    "Adaptive EMA Crossover": EmaCrossAtrStrategy,
}

PARAM_SCHEMA_REGISTRY = {
    "MA_Cross": EmaCrossAtrParams,
    "Bollinger_Bands": RsiBbMeanReversionParams,
    "RSI_MACD_Crossover": MacdAdxTrendParams,
    "Volatility_Squeeze": VolatilitySqueezeParams,
    "SuperTrend_ADX_Filter": SuperTrendAdxParams,
    "Ichimoku_Cloud_Breakout": IchimokuBreakoutParams,
    "Smart_Money_Concepts": SmcOrderBlockFvgParams,
    "AI_Signal_Confirmation": AiEnhancedSignalParams,
    "Optimizer_Portfolio": OptimizerPortfolioParams,
}
        ...
        if info:
            return {
                ...
                "details": {
                    ...
                    "balance": float(info.balance), # Ensure float for JSON serialization
...
class TradingViewClient:
    ...
    async def fetch_data_for_backtest(self, symbol: str, interval_str: str, start_date_str: str, end_date_str: str) -> Optional[pd.DataFrame]:
        """A dedicated method for fetching large historical datasets for backtesting."""
        from tvDatafeed import TvDatafeed, Interval # Local import
        
        # TradingView uses a different interval format
        interval_map = {'1D': Interval.in_daily, '4H': Interval.in_4_hour, '1H': Interval.in_1_hour}
        
        tv = TvDatafeed(self.username, self.password)
        
        # Calculate number of bars needed. This is an approximation.
        ...
        if days_needed <= 0: return None
        
        try:
            df = await asyncio.to_thread(
                tv.get_hist,
                symbol=symbol.replace('/', ''),
                exchange='BINANCE', # Or another major exchange
                interval=interval_map.get(interval_str, Interval.in_daily),
                n_bars=days_needed + 5 # Fetch a few extra bars
            )
            ...
            logger.error(f"TradingView backtest data fetch failed: {e}")
            return None
...
class SwapService:
    ...
    async def execute_swap(self, db: AsyncSession, user_id: str, req: SwapExecuteRequest) -> SwapExecuteResponse:
        ...
        async with db.begin():
            try:
                # Force high precision for financial calculations
                amount_in = Decimal(str(validated_quote.amount_in))
                amount_out = Decimal(str(validated_quote.amount_out))
                
                # Debit
                await wallet_service._update_balance(
                    db, user_id, validated_quote.from_asset, -amount_in,
                    TransactionType.SWAP, notes=f"Swap to {validated_quote.to_asset}"
                )

                # Credit
                tx = await wallet_service._update_balance(
                    db, user_id, validated_quote.to_asset, amount_out,
                    TransactionType.SWAP,
                    notes=f"Swap from {validated_quote.from_asset}",
                    related_asset=validated_quote.from_asset, related_amount=-amount_in,
...
class MLService:
    ...
    def analyze_market_conditions_from_tv(self, tv_analysis: Any) -> Dict[str, Any]:
        """
        ...
        """
        # --- FIX: Check if the object exists and has the required attribute ---
        if not tv_analysis or not hasattr(tv_analysis, "summary"):
            return {"score": 0, "summary": "Data Unavailable", "prediction": "N/A", "indicators": {}}

        # 1. Map TradingView's recommendation to our summary
        # Access attributes with dot notation, not ["key"]
        ...
        # tv_summary is a dict-like object or dict, handle access safely
        recommendation = tv_summary.get("RECOMMENDATION") if isinstance(tv_summary, dict) else getattr(tv_summary, "RECOMMENDATION", "NEUTRAL")
        summary = summary_map.get(recommendation, "Neutral")
        ...
        # 3. Extract key indicator values
        # TradingView 'indicators' is usually a dict
        tv_indicators = tv_analysis.indicators
        
        indicators = {
            "RSI (14)": round(tv_indicators.get("RSI", 0), 2),
            "MACD Level": round(tv_indicators.get("MACD.macd", 0), 2),
            "Volatility": tv_indicators.get("Volatility.D", "N/A")
        }
...
class StrategyAnalysisService:
    ...
    async def backtest_strategy(self, strategy_name: str, params: dict, symbol: str, exchange_name: str,
        ...
        if df is None or df.empty:
            logger.warning("All CCXT providers failed. Falling back to TradingView for backtest data.")
            try:
                # We need a way to get the tv_client instance here. Assuming it's on app.state
                # from the lifespan manager.
                # In a Celery task, this is harder. Let's instantiate it directly.
                from .main import TradingViewClient # Local import
                tv_client = TradingViewClient()
                df = await tv_client.fetch_data_for_backtest(symbol, '1D', start_date, end_date) # New method
                await tv_client.close_session()
                if df is None or df.empty:
                    raise ValueError("TradingView fallback also returned no data.")
                logger.info(f"Successfully fetched {len(df)} records via TradingView fallback.")
            except Exception as e:
                logger.error(f"FATAL: All data sources failed, including TradingView fallback. Error: {e}")
                raise HTTPException(status_code=503, detail="All market data providers are currently unavailable.")
...
class StrategyService:
    ...
    async def start_bot(self, db: AsyncSession, user: User, bot: TradingBot, background_tasks: BackgroundTasks):
        """
        Robustly starts a trading bot with immediate UI feedback and self-healing capabilities.
        """
        async with self._bot_locks[bot.id]:
            ...
            if not current_bot or current_bot.is_active or current_bot.id in self.running_bot_tasks:
                logger.warning(f"Bot {bot.id} start request ignored (already active or invalid).")
                return

            # --- FEEDBACK: Immediate Boot Log ---
            await websocket_manager.send_personal_message(
                {"type": "bot_log", "bot_id": str(bot.id), "message": "🚀 System: Booting up strategy engine..."},
                user.id
            )
            ...
            try:
                ...
                if current_bot.exchange in [ExchangeName.MT4.value, ExchangeName.MT5.value]:
                    ...
                    mt5_creds = await db.scalar(select(MT5Credentials).where(MT5Credentials.user_id == user.id))

                    if not mt5_creds:
                        err_msg = "MT5 Credentials not found. Please configure them in Settings > Integrations."
                        await websocket_manager.send_personal_message(
                            {"type": "error", "bot_id": str(bot.id), "message": err_msg}, user.id
                        )
                        return
                    ...
                    # --- Attempt Connection ---
                    await websocket_manager.send_personal_message(
                        {"type": "bot_log", "bot_id": str(bot.id), "message": "🔌 Connecting to MT5 Terminal..."}, user.id
                    )
                    
                    try:
                        connected = await mt5_gateway_service.connect_and_login(
                            login=int(mt5_creds.account_number),
                            password=user_service.decrypt_data(mt5_creds.password_encrypted),
                    ...
                    if not connected:
                        err_msg = "Failed to connect to MT5 Terminal. Please check if the terminal is open."
                        await websocket_manager.send_personal_message(
                            {"type": "error", "bot_id": str(bot.id), "message": err_msg}, user.id
                        )
                        return
                    ...
                    await telegram_service.notify_user(user.id, f"🚀 Bot `{current_bot.name}` (MT5) is active.")
                    await websocket_manager.send_personal_message(
                        {"type": "bot_status", "bot_id": str(current_bot.id), "status": "started"}, user.id)
                    await websocket_manager.send_personal_message(
                        {"type": "bot_log", "bot_id": str(current_bot.id), "message": "✅ MT5 Bridge Connected. Waiting for terminal signals..."}, 
                        user.id
                    )
                    return # Exit, as MT5 bots don't run a Python loop
                ...
                if current_bot.strategy_name == "TradingView_Alert":
                    ...
                    msg = (
                        f"🚀 Bot `{current_bot.name}` is listening.\n"
                        f"Webhook URL: `{webhook_url}`"
                    )
                    await telegram_service.notify_user(user.id, msg)
                    
                    await websocket_manager.send_personal_message(
                        {"type": "bot_status", "bot_id": str(current_bot.id), "status": "started", "webhook_url": webhook_url}, 
                        user.id
                    )
                    await websocket_manager.send_personal_message(
                        {"type": "bot_log", "bot_id": str(current_bot.id), "message": "✅ Webhook Listener Active. Waiting for alerts..."},
                        user.id
                    )
                    return # Exit, as webhook bots are event-driven

                # 4. Internal Strategy Logic
                ...
                # --- FEEDBACK: Connection Log ---
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": f"📡 Connecting to {current_bot.exchange} websocket feed for {current_bot.symbol}..."},
                    user.id
                )
                ...
                # 6. Define the Self-Healing Wrapper
                async def safe_strategy_runner():
                    # --- FEEDBACK: Loop Start ---
                    await websocket_manager.send_personal_message(
                        {"type": "bot_log", "bot_id": str(bot.id), "message": "✅ Strategy Loop Active. Waiting for candle data..."},
                        user.id
                    )
                    ...
                    while True:
                        try:
                            # Database Check (ensure bot wasn't stopped via API while loop was running)
                            async with async_session_maker() as check_db:
                                check_bot = await check_db.get(TradingBot, bot.id)
                                if not check_bot or not check_bot.is_active:
                                    logger.info(f"Bot {bot.id} marked inactive. Stopping loop.")
                                    break

                            # EXECUTE STRATEGY LOGIC
                            await strategy_func(user, bot, data_queue, background_tasks)
                            
                            # If strategy_func returns properly (e.g. stop requested), we break
                            break 

                        except asyncio.CancelledError:
                            logger.info(f"Bot {bot.id} task cancelled.")
                            break
                        except Exception as e:
                            error_count += 1
                            logger.error(f"Bot {bot.id} crashed (Attempt {error_count}): {e}", exc_info=True)
                            
                            # Notify UI of crash and retry
                            await websocket_manager.send_personal_message(
                                {"type": "error", "bot_id": str(bot.id), "message": f"⚠️ Strategy crashed: {str(e)}. Retrying in 5s..."},
                                user.id
                            )
                            ...
                            if error_count > 10:
                                await websocket_manager.send_personal_message(
                                    {"type": "error", "bot_id": str(bot.id), "message": "🛑 Too many consecutive errors. Bot shutting down."},
                                    user.id
                                )
                                await telegram_service.notify_user(user.id, f"🛑 Bot `{bot.name}` stopped after critical failures.")
                                # Update DB state to inactive
                                async with async_session_maker() as fail_db:
                                    fail_bot = await fail_db.get(TradingBot, bot.id)
                                    if fail_bot:
                ...
                # 7. Launch Task
                self.running_bot_tasks[current_bot.id] = asyncio.create_task(safe_strategy_runner())
                ...
                await db.commit()
                await websocket_manager.send_personal_message(
                    {"type": "error", "message": f"Failed to start bot: {e}"}, user.id)
    ...
    async def run_visual_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue,
        ...
        historical_candles = deque(maxlen=250)

        # --- Hydration ---
        try:
            client = await exchange_manager.get_private_client(user.id, bot.exchange, AssetClass(bot.asset_class), MarketType.SPOT)
            if client:
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": "📥 Fetching historical data..."}, user.id)
                ...
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": f"✅ Hydrated with {len(historical_candles)} candles."}, user.id)
        except Exception as e:
            logger.error(f"Hydration failed: {e}")

        # --- Main Loop ---
        try: # <--- CRITICAL FIX: ADDED TRY HERE
            while True:
                ...
                if len(historical_candles) < 200:
                    if len(historical_candles) % 10 == 0:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"⏳ Gathering data: {len(historical_candles)}/200..."}, user.id)
                    continue
                ...
                try:
                    ...
                    signal = interpreter.evaluate() # 'buy', 'sell', 'hold'
                    
                    # --- Heartbeat Log ---
                    log_msg = f"📊 Visual Logic Result: {signal.upper()} | Price ${candle['close']:.2f}"
                    await websocket_manager.send_personal_message(
                        {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)
                    ...
                    async with async_session_maker() as db:
                        ...
                        if signal == 'buy' and not in_position:
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 Visual Logic Triggered BUY!"}, user.id)
                            await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(candle['close'])), background_tasks)
                        elif signal == 'sell' and in_position:
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🔴 Visual Logic Triggered SELL!"}, user.id)
                            await self.execute_bot_trade(db, user, current_bot, 'sell', Decimal(str(candle['close'])), background_tasks)

                except Exception as e:
                    logger.error(f"Visual strategy error: {e}")
                    await websocket_manager.send_personal_message(
                        {"type": "error", "bot_id": str(bot.id), "message": f"Logic Error: {e}"}, user.id)
    ...
    async def run_rsi_macd_crossover_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue,
        ...
        from collections import deque
        required_history = max(rsi_period, macd_slow) + 15
        ...
        try:
            ...
            if client:
                ...
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": f"✅ Hydrated with {len(historical_closes)} candles."}, user.id)
        except Exception as e:
            logger.error(f"Hydration failed: {e}")
        ...
        try: # <--- ADDED TRY HERE
            while True:
                ...
                if len(historical_closes) < required_history:
                    if len(historical_closes) % 5 == 0:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"⏳ Gathering data: {len(historical_closes)}/{required_history} candles..."}, user.id)
                    continue
                ...
                current_macd_val = macd_line.iloc[-1]
                current_signal_val = signal_line.iloc[-1]
                current_macd_hist_state = 1 if current_macd_val > current_signal_val else -1

                # Heartbeat Log
                log_msg = f"📊 Analysis: Price ${close_price:.2f} | RSI: {current_rsi:.1f} | MACD: {current_macd_val:.4f}"
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)
                ...
                async with async_session_maker() as db:
                    ...
                    if buy_signal and not in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 Oversold + MACD Cross! BUYING..."}, user.id)
                        ...
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🔴 Overbought + MACD Cross! SELLING..."}, user.id)
        ...
        try:  # <--- THIS WAS MISSING
            while True:
                ...
                if len(historical_closes) < long_window:
                    if len(historical_closes) % 10 == 0:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"⏳ Gathering data: {len(historical_closes)}/{long_window} candles..."}, user.id)
                    continue
                ...
                # 3. Heartbeat
                log_msg = f"📊 Analysis: Price ${close_price:.2f} | MA({short_window}): {current_short_ma:.2f} | MA({long_window}): {current_long_ma:.2f}"
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, 
                    user.id
                )
                ...
                async with async_session_maker() as db:
                    ...
                    if buy_signal and not in_position:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 GOLDEN CROSS DETECTED! Executing BUY..."},
                            user.id
                        )
                        await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(kline['c'])), background_tasks)
                        ...
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": "🔴 DEATH CROSS DETECTED! Executing SELL..."},
                            user.id
                        )
                        await self.execute_bot_trade(db, user, current_bot, 'sell', Decimal(str(kline['c'])), background_tasks)
        ...
        try: # <--- ADDED TRY HERE
            while True:
                ...
                if len(historical_closes) < window:
                    if len(historical_closes) % 5 == 0:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"⏳ Gathering data: {len(historical_closes)}/{window} candles..."}, user.id)
                    continue
                ...
                # Heartbeat Log
                log_msg = f"📊 Analysis: Price ${close_price:.2f} | Upper: {upper_band:.2f} | Lower: {lower_band:.2f}"
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)
                ...
                async with async_session_maker() as db:
                    ...
                    if buy_signal and not in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 Price below Lower Band. Reversion BUY..."}, user.id)
                        await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(kline['c'])), background_tasks)
                    elif sell_signal and in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🔴 Price above Upper Band. Reversion SELL..."}, user.id)
                        await self.execute_bot_trade(db, user, current_bot, 'sell', Decimal(str(kline['c'])), background_tasks)
        ...
        try: # <--- ADDED TRY HERE
            while True:
                ...
                if len(historical_candles) < 50:
                    if len(historical_candles) % 10 == 0:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"⏳ Gathering data: {len(historical_candles)}/50 candles..."}, user.id)
                    continue
                ...
                # Heartbeat Log
                structure = "BOS (Bullish)" if latest['bos'] == 1 else ("BOS (Bearish)" if latest['bos'] == -1 else "Neutral")
                ob_level = latest['bullish_ob'] if latest['bullish_ob'] > 0 else "None"
                log_msg = f"📊 Analysis: Price ${candle['close']:.2f} | Structure: {structure} | Bullish OB: {ob_level}"
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)
                ...
                async with async_session_maker() as db:
                    ...
                    if not in_position:
                        # Logic: 1. Bullish Change of Character detected recently. 2. Price dips into Bullish Order Block.
                        bullish_choch = (df['choch'] == 1).rolling(10).sum().iloc[-1] > 0
                        in_zone = latest['close'] <= latest['bullish_ob'] and latest['bullish_ob'] > 0
                        
                        if bullish_choch and in_zone:
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 SMC Entry Validated! BUYING..."}, user.id)
                            await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(candle['close'])), background_tasks)
    ...
    async def run_grid_trading_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue = None, background_tasks: BackgroundTasks = None):
        # NOTE: Grid trading manages its own loop via API polling, not the data_queue.
        params = json.loads(bot.strategy_params)
        upper_price = Decimal(str(params.get('upper_price')))
        lower_price = Decimal(str(params.get('lower_price')))
        num_grids = int(params.get('num_grids', 10))
        trade_amount_base = Decimal(str(params.get('trade_amount_base')))

        # --- Feedback ---
        await websocket_manager.send_personal_message(
            {"type": "bot_log", "bot_id": str(bot.id), "message": f"🕸️ Initializing Grid: {num_grids} levels between ${lower_price} and ${upper_price}."}, user.id)
        ...
        try:
            # We need the raw CCXT client for grid management
            broker_adapter = await exchange_manager.get_private_client(user.id, bot.exchange, AssetClass(bot.asset_class), MarketType.SPOT)
            if not isinstance(broker_adapter, CcxtClient):
                raise ConnectionError("Grid trading requires a CCXT-compatible exchange.")
            
            private_exchange = broker_adapter._client 

            # 1. Setup
            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🧹 Canceling existing open orders for symbol..."}, user.id)
            await private_exchange.cancel_all_orders(bot.symbol)
            
            ticker = await private_exchange.fetch_ticker(bot.symbol)
            current_price = Decimal(str(ticker['last']))

            # 2. Place Initial Grid
            live_orders = {}
            for price in grid_lines:
                side = 'buy' if price < current_price else 'sell'
                # Skip if price is too close to current (avoid immediate fill taker fees if possible)
                if abs(price - current_price) / current_price < 0.002: continue

                try:
                    order = await private_exchange.create_limit_order(bot.symbol, side, float(trade_amount_base), float(price))
                    live_orders[order['id']] = order
                    await websocket_manager.send_personal_message(
                        {"type": "bot_log", "bot_id": str(bot.id), "message": f"placed {side.upper()} grid at ${price:.2f}"}, user.id)
                    await asyncio.sleep(0.2) # Rate limit protection
                except Exception as e:
                    logger.error(f"Grid order failed: {e}")

            await websocket_manager.send_personal_message(
                {"type": "bot_log", "bot_id": str(bot.id), "message": f"✅ Grid Active. Monitoring {len(live_orders)} orders..."}, user.id)
            ...
            while True:
                ...
                try:
                    ...
                    if not filled_order_ids:
                        # Heartbeat
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": "zzz... No grid levels hit."}, user.id)
                        continue
                    ...
                    for order_id in filled_order_ids:
                        ...
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"💰 Grid Level Hit! {filled_side.upper()} filled at ${filled_price}."}, user.id)
                        ...
                        if lower_price <= new_price <= upper_price:
                            new_order = await private_exchange.create_limit_order(bot.symbol, new_side, float(trade_amount_base), float(new_price))
                            live_orders[new_order['id']] = new_order
                            await websocket_manager.send_personal_message(
                                {"type": "bot_log", "bot_id": str(bot.id), "message": f"🔄 Replacing grid: {new_side.upper()} at ${new_price:.2f}"}, user.id)

                except Exception as e:
                    logger.error(f"Grid monitor error: {e}")
                    await websocket_manager.send_personal_message(
                        {"type": "error", "bot_id": str(bot.id), "message": f"Grid polling error: {str(e)}"}, user.id)

        except asyncio.CancelledError:
            logger.info("Grid task cancelled")
        except Exception as e:
            ...
            if private_exchange:
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": "🛑 Stopping Grid. Cancelling open orders..."}, user.id)
                try:
                    await private_exchange.cancel_all_orders(bot.symbol)
                except: pass
    ...
    async def run_volatility_squeeze_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue,
        ...
        from collections import deque
        required_history = max(bb_period, kc_period) + 10
        ...
        # --- Main Loop ---
        try: # <--- CRITICAL FIX: Added this try block
            while True:
                kline = await data_queue.get()
                candle = {'timestamp': kline['t'], 'open': float(kline['o']), 'high': float(kline['h']),
                          'low': float(kline['l']), 'close': float(kline['c']), 'volume': float(kline['v'])}
                ...
                if len(historical_candles) < required_history:
                    if len(historical_candles) % 5 == 0:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"⏳ Gathering data: {len(historical_candles)}/{required_history} candles..."}, user.id)
                    continue

                df = pd.DataFrame(list(historical_candles))
                df.ta.bbands(length=bb_period, std=bb_std, append=True)
                df.ta.kc(length=kc_period, scalar=kc_atr_mult, append=True)
                latest = df.iloc[-1]
                ...
                # --- Heartbeat Log ---
                status = "ON (Accumulating)" if squeeze_is_on else "OFF"
                log_msg = f"📊 Analysis: Price ${candle['close']:.2f} | Squeeze: {status}"
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)
                ...
                async with async_session_maker() as db:
                    ...
                    if buy_signal and not in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 Squeeze Released Upward! BUYING..."}, user.id)
                        await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(candle['close'])), background_tasks)
    ...
    async def run_supertrend_adx_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue,
        ...
        from collections import deque
        required_history = max(st_period, adx_period) + 15
        historical_candles = deque(maxlen=required_history)
        ...
        try: # <--- CRITICAL FIX
            while True:
                ...
                # Logic
                adx_val = latest[f'ADX_{adx_period}']
                is_trending = adx_val > adx_threshold
                st_col = f'SUPERTd_{st_period}_{st_multiplier}'
                
                # --- Heartbeat Log ---
                trend_str = "TRENDING" if is_trending else "RANGING"
                # Check if supertrend column exists before accessing
                if st_col in latest:
                    dir_str = "BULLISH" if latest[st_col] == 1 else "BEARISH"
                    log_msg = f"📊 Analysis: ADX: {adx_val:.1f} ({trend_str}) | SuperTrend: {dir_str}"
                    await websocket_manager.send_personal_message(
                        {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)

                    buy_flip = latest[st_col] == 1 and prev[st_col] == -1
                    sell_flip = latest[st_col] == -1 and prev[st_col] == 1
                    ...
                    async with async_session_maker() as db:
                        ...
                        if buy_signal and not in_position:
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 Trend Start (Buy) Detected!"}, user.id)
                            await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(candle['close'])), background_tasks)
                        elif sell_signal and in_position:
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🔴 Trend Flip (Sell) Detected!"}, user.id)
                            await self.execute_bot_trade(db, user, current_bot, 'sell', Decimal(str(candle['close'])), background_tasks)
    ...
    async def run_ichimoku_breakout_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue,
        ...
        senkou = params.get('senkou_period', 52)

        from collections import deque
        required_history = senkou + 26 + 10 # Senkou B shift + buffer
        historical_candles = deque(maxlen=required_history)
        ...
        try: # <--- CRITICAL FIX
            while True:
                ...
                df = pd.DataFrame(list(historical_candles))
                df.ta.ichimoku(tenkan=tenkan, kijun=kijun, senkou=senkou, append=True)
                latest = df.iloc[-1]
                prev = df.iloc[-2]

                # Cloud Components
                span_a = latest[f'ISA_{tenkan}']
                span_b = latest[f'ISB_{kijun}']
                cloud_top = max(span_a, span_b)
                cloud_bottom = min(span_a, span_b)
                price = latest['close']

                # --- Heartbeat Log ---
                pos = "ABOVE Cloud" if price > cloud_top else ("BELOW Cloud" if price < cloud_bottom else "INSIDE Cloud")
                log_msg = f"📊 Analysis: Price ${price:.2f} | {pos}"
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)

                # Breakout Logic
                bullish_breakout = price > cloud_top and prev['close'] <= max(prev[f'ISA_{tenkan}'], prev[f'ISB_{kijun}'])
                bearish_breakout = price < cloud_bottom and prev['close'] >= min(prev[f'ISA_{tenkan}'], prev[f'ISB_{kijun}'])
                ...
                async with async_session_maker() as db:
                    ...
                    if bullish_breakout and not in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🟢 Kumo Breakout UP! BUYING..."}, user.id)
                        await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(candle['close'])), background_tasks)
                    elif bearish_breakout and in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🔴 Kumo Breakout DOWN! SELLING..."}, user.id)
                        await self.execute_bot_trade(db, user, current_bot, 'sell', Decimal(str(candle['close'])), background_tasks)
    ...
    async def run_ai_enhanced_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue,
                                       background_tasks: BackgroundTasks):
        params = json.loads(bot.strategy_params)
        confidence_threshold = params.get('confidence_threshold', 0.2)
        fast_ema = 10
        slow_ema = 30

        from collections import deque
        required_history = 200 # Needed for AI feature engineering
        historical_candles = deque(maxlen=required_history + 5)
        ...
        try:
            ...
            if client:
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": "📥 Fetching historical data for AI..."}, user.id)
                initial_ohlcv = await client.fetch_ohlcv(bot.symbol, '1m', required_history)
                await client.close()
                for t, o, h, l, c, v in initial_ohlcv:
                    historical_candles.append({'timestamp': t, 'open': o, 'high': h, 'low': l, 'close': c, 'volume': v})
        ...
        try: # <--- CRITICAL FIX
            while True:
                ...
                if len(historical_candles) < required_history:
                    if len(historical_candles) % 10 == 0:
                        await websocket_manager.send_personal_message(
                            {"type": "bot_log", "bot_id": str(bot.id), "message": f"⏳ Gathering data: {len(historical_candles)}/{required_history} for AI model..."}, user.id)
                    continue

                df = pd.DataFrame(list(historical_candles))
                
                # Base Logic (EMA Cross)
                df['ema_fast'] = df['close'].ewm(span=fast_ema, adjust=False).mean()
                df['ema_slow'] = df['close'].ewm(span=slow_ema, adjust=False).mean()
                latest = df.iloc[-1]
                prev = df.iloc[-2]
                
                base_buy = latest['ema_fast'] > latest['ema_slow'] and prev['ema_fast'] <= prev['ema_slow']
                base_sell = latest['ema_fast'] < latest['ema_slow'] and prev['ema_fast'] >= prev['ema_slow']

                # --- Heartbeat Log ---
                log_msg = f"📊 Analysis: Price ${candle['close']:.2f} | AI Ready."
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)

                if not base_buy and not base_sell:
                    continue

                # Run AI only on signal
                await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": "🤖 Base Signal detected. Querying Optimus AI..."}, user.id)
                ai_result = ml_service.analyze_market_conditions(df)
                ...
                async with async_session_maker() as db:
                    ...
                    if base_buy and not in_position:
                        if ai_score > confidence_threshold:
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": f"🟢 AI Confirmed (Score: {ai_score:.2f})! BUYING..."}, user.id)
                            await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(candle['close'])), background_tasks)
                        else:
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": f"⛔ AI Vetoed Buy (Score: {ai_score:.2f} < {confidence_threshold})."}, user.id)

                    elif base_sell and in_position:
                        if ai_score < -confidence_threshold: # Confidence in bearishness
                            await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": f"🔴 AI Confirmed (Score: {ai_score:.2f})! SELLING..."}, user.id)
                            await self.execute_bot_trade(db, user, current_bot, 'sell', Decimal(str(candle['close'])), background_tasks)
    ...
    async def run_optimizer_portfolio_strategy(self, user: User, bot: TradingBot, data_queue: asyncio.Queue,
        ...
        for name in strategy_pool_names:
            ...
            if StrategyClass and name != "Optimizer_Portfolio":
                # Initialize with empty state and default params
                sub_strategies.append(StrategyClass(0, bot.symbol, '1m', {}, {}))
        ...
        try:
            ...
            if client:
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": "📥 Fetching historical data for Portfolio..."}, user.id)
                initial_ohlcv = await client.fetch_ohlcv(bot.symbol, '1m', required_history)
                await client.close()
                for t, o, h, l, c, v in initial_ohlcv:
                    historical_candles.append({'timestamp': t, 'open': o, 'high': h, 'low': l, 'close': c, 'volume': v})
        ...
        try: # <--- CRITICAL FIX: ADDED TRY HERE
            while True:
                ...
                for sub in sub_strategies:
                    sub.update_data(df.copy()) # Pass copy to avoid conflicts
                    signal_obj = sub.generate_signal()
                    if signal_obj.action == "BUY":
                        buy_votes += 1
                        details.append(f"{type(sub).__name__}: BUY")
                    elif signal_obj.action == "SELL":
                        sell_votes += 1
                        details.append(f"{type(sub).__name__}: SELL")
                ...
                # --- Heartbeat Log ---
                log_msg = f"🗳️ Votes: {buy_votes} BUY / {sell_votes} SELL | Trend: {trend_str}"
                await websocket_manager.send_personal_message(
                    {"type": "bot_log", "bot_id": str(bot.id), "message": log_msg}, user.id)

                # Decision
                final_buy = buy_votes >= min_confluence and is_uptrend
                final_sell = sell_votes >= min_confluence and not is_uptrend
                ...
                async with async_session_maker() as db:
                    ...
                    if final_buy and not in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": f"🏆 Confluence Reached ({buy_votes} Votes)! BUYING..."}, user.id)
                        await self.execute_bot_trade(db, user, current_bot, 'buy', Decimal(str(candle['close'])), background_tasks)
                    elif final_sell and in_position:
                        await websocket_manager.send_personal_message({"type": "bot_log", "bot_id": str(bot.id), "message": f"🏆 Confluence Reached ({sell_votes} Votes)! SELLING..."}, user.id)
                        await self.execute_bot_trade(db, user, current_bot, 'sell', Decimal(str(candle['close'])), background_tasks)
...
async def broadcast_market_data():
    ...
    while True:
        ...
        if exchange:
            source = exchange.id
            try:
                # FIX: Removed invalid params={'type': 'spot'} which caused Binance -1104 error
                ...
                if available_symbols:
                    ...
                    for symbol, data in fetched_tickers.items():
                        if data and data.get('last') is not None:
                            market_data[symbol] = {"price": data['last'], "change": data.get('percentage', 0)}
        ...
        if not market_data:
            # Add delay to prevent hitting TV rate limits immediately after a CCXT failure
            await asyncio.sleep(2) 
            source = "TradingView"
            try:
                tv_client_temp = TradingViewClient()
                # Use a reduced list for fallback to be safer on rate limits
                fallback_symbols = ['BTC/USDT', 'ETH/USDT'] 
                fetched_tickers = await tv_client_temp.fetch_tickers(fallback_symbols)
...
async def lifespan(app: FastAPI):
    ...
    async with engine.begin() as conn:
        # This command tells SQLAlchemy to look at all your classes that inherit
        # from `Base` and create the corresponding tables if they don't exist.
        await conn.run_sync(Base.metadata.create_all)
    logger.info("Database tables verified/created successfully.")

    # --- THIS IS THE LOGIC THAT CREATES THE SUPERUSER ROW ---
    logger.info("Checking for superuser account...")
    async with async_session_maker() as session:
        async with session.begin():
            ...
            if result.scalar_one_or_none() is None:
                # If not, create the superuser now that the 'users' table exists
                superuser = User(
...
async def get_user_portfolio(
    ...
    FIX: Applies fallback rates even if market data providers are offline.
    """
    fetch_tasks = []

    # 1. MT5 Task
    mt5_creds = await db.scalar(select(MT5Credentials).where(MT5Credentials.user_id == current_user.id))
    if mt5_creds:
        fetch_tasks.append(fetch_mt5_balance(mt5_creds)) 
    ...
    for result in source_balances:
        ...
        if not result or "balances" not in result: continue
        ...
        for asset, amount in result["balances"].items():
            try:
                ...
                if d_amount > 0:
                    aggregated_portfolio[asset]['amount'] += d_amount
        ...
        if pricing_exchange:
            try:
                ...
                for asset in assets_to_price:
                    ...
                    symbol = f"{asset}/USDT"
                    if symbol in pricing_exchange.symbols:
                        try:
                            ticker = await pricing_exchange.fetch_ticker(symbol)
                            asset_prices[asset] = Decimal(str(ticker['last']))
                        except: pass
            except Exception as e:
                logger.warning(f"Live pricing failed, using fallbacks: {e}")
            finally:
                await pricing_exchange.close()

    # 5. Build Final Portfolio
    final_portfolio = []
    ...
    for asset, data in aggregated_portfolio.items():
        ...
        elif asset in FALLBACK_RATES:
            price = FALLBACK_RATES[asset]
        else:
            price = Decimal(0) # Unknown asset
            
        usd_value = amount * price

        # FIX: Include item if amount > 0, even if price is 0. 
        # This ensures users see their assets even if pricing fails.
        if amount > 0:
...
async def fetch_asset_price(exchange: ccxt.Exchange, symbol: str) -> float:
    # This helper is no longer used by the new logic above, 
    # but keep it if other parts of your code need it.
    try:
        ticker = await exchange.fetch_ticker(symbol)
        return ticker['last']
    except Exception:
...
async def fetch_mt5_balance(creds: MT5Credentials = None) -> Dict:
    if not creds:
        return {"source": "MetaTrader 5", "balances": {}}
        
    try:
        # Create a temporary client for this fetch
        client = Mt5Client(int(creds.account_number), user_service.decrypt_data(creds.password_encrypted), creds.server)
        summary = await asyncio.to_thread(client.get_account_summary)
        
        if summary and summary.get("status") == "success":
            return {
                "source": "MetaTrader 5",
                "balances": {summary["details"]["currency"]: summary["details"]["balance"]}
            }
    except Exception as e:
        logger.error(f"MT5 Portfolio fetch failed: {e}")
...
@public_router.post("/serials/activate", response_model=SerialNumberActivateResponse)
async def activate_serial_number(request: SerialNumberActivateRequest, http_request: Request, db: AsyncSession = Depends(get_db)):
    """
    Validates a serial number against the logged-in user, their plan, machine ID, and IP.
    This is the definitive, multi-factor activation endpoint.
    """
    client_ip = http_request.client.host

    async with db.begin():
        # 1. Find the serial key in the database
        serial = await db.scalar(
            select(SerialNumber).where(SerialNumber.serial_key == request.serial_key)
        )
        ...
        if serial.assigned_user_id != current_user.id:
            raise HTTPException(status_code=403, detail="This serial key is not assigned to your account.")

        if not current_user.is_subscription_active() or current_user.subscription_plan != serial.associated_plan:
            raise HTTPException(status_code=403,
                                detail=f"This serial key requires an active '{serial.associated_plan}' subscription. Your current plan is '{current_user.subscription_plan}'.")

        # Hash the incoming IDs for secure storage and comparison
        machine_id_hash = pwd_context.hash(request.machine_id)
        ip_hash = pwd_context.hash(client_ip)

        if serial.machine_id_hash:
            # Key is already activated. Verify the machine and IP.
            if not pwd_context.verify(request.machine_id, serial.machine_id_hash):
                raise HTTPException(status_code=403, detail="This serial key has been activated on another machine.")

            # Optional: Check if the IP has changed significantly.
            # You might allow some flexibility here (e.g., allow 1-2 different IPs).
            if not pwd_context.verify(client_ip, serial.last_ip_hash):
                logger.warning(
                    f"Activation for user {current_user.email} from new IP. Previous hash: {serial.last_ip_hash}")
                # For now, we'll just update the IP hash. In a stricter system, you might
                # require re-validation or alert the user.
                serial.last_ip_hash = ip_hash

        else:
            # First-time activation: lock the key to this machine and IP.
            serial.machine_id_hash = machine_id_hash
            serial.last_ip_hash = ip_hash
            serial.activated_at = datetime.datetime.now(datetime.timezone.utc)

        # If all checks pass, create the short-lived activation token
        activation_token = create_access_token(
            data={"sub": current_user.id, "scope": "activation"},
            expires_delta=datetime.timedelta(minutes=5)
        )

        await db.commit()

    return SerialNumberActivateResponse(
        message="Serial key validated successfully. Please complete the login.",
        activation_token=activation_token
    )


@public_router.post("/serials/activate-desktop", response_model=Token)
async def activate_desktop_serial(
    request: SerialNumberActivateRequest,
    http_request: Request, # Used to get the client's IP address
    db: AsyncSession = Depends(get_db)
):
    """
    A dedicated, all-in-one endpoint for the desktop app. It validates a serial key
    against multiple factors (user, plan, machine, IP) and immediately returns 
    long-lived session tokens upon successful activation.
    """
    client_ip = http_request.client.host
    
    async with db.begin():
        # --- Step 1: Find the Serial Key and Eagerly Load the Assigned User ---
        # `selectinload` is a performance optimization to fetch the user in the same query.
        query = (
            select(SerialNumber)
            .options(selectinload(SerialNumber.assigned_user))
            .where(SerialNumber.serial_key == request.serial_key)
        )
        result = await db.execute(query)
        serial = result.scalar_one_or_none()

        # --- Step 2: Perform Multi-Factor Validation ---

        # Check 2a: Does the key exist and is it marked as active?
        if not serial or not serial.is_active:
            raise HTTPException(status_code=404, detail="Serial key is invalid, expired, or has been deactivated.")
        
        # The user object is now pre-loaded thanks to `selectinload`.
        assigned_user = serial.assigned_user
        if not assigned_user:
            # This indicates data inconsistency, which should be a server error.
            logger.error(f"Data integrity error: Serial key {serial.id} is not assigned to a valid user.")
            raise HTTPException(status_code=500, detail="Internal server error: Serial key is unassigned.")
            
        # Check 2b: Is the user's subscription active and for the correct plan?
        if not assigned_user.is_subscription_active() or assigned_user.subscription_plan != serial.associated_plan:
            raise HTTPException(
                status_code=403, 
                detail=f"This serial key requires an active '{serial.associated_plan}' subscription. Your account's subscription is either inactive or for a different plan."
            )
        ...
        if serial.machine_id_hash:
            # --- Step 3a: Logic for an ALREADY ACTIVATED Key ---

            # Check 3a.1: Does the machine ID match?
            if not pwd_context.verify(request.machine_id, serial.machine_id_hash):
                logger.warning(f"Failed activation attempt for user {assigned_user.email} with serial {serial.serial_key} from a different machine.")
                raise HTTPException(status_code=403, detail="This serial key has already been activated on another machine.")
            
            # Check 3a.2 (Optional but recommended): Log significant IP changes.
            if not pwd_context.verify(client_ip, serial.last_ip_hash):
                 logger.warning(f"Activation for user {assigned_user.email} from new IP address: {client_ip}. Previous hash: {serial.last_ip_hash}")
                 # Update the last known IP hash.
                 serial.last_ip_hash = ip_hash
            
            logger.info(f"Successful re-activation for user {assigned_user.email} on a known machine.")

        else:
            # --- Step 3b: Logic for a FIRST-TIME Activation ---
            logger.info(f"First-time activation for user {assigned_user.email} with serial {serial.serial_key}.")
            
            # Lock the key to this machine and IP address.
            serial.machine_id_hash = machine_id_hash
            serial.last_ip_hash = ip_hash
            serial.activated_at = datetime.datetime.now(datetime.timezone.utc)
        
        # --- Step 4: Generate Final, Long-Lived Session Tokens ---
        # If all validation checks pass, we can grant access.
        
        user = assigned_user
        access_token = create_access_token(data={"sub": user.id, "ver": user.token_version, "role": user.role})
        refresh_token = create_refresh_token(data={"sub": user.id})

        # The `async with db.begin()` block will automatically commit all changes here.
...
class TradingBotUpdate(BaseModel):
    ...
    # --- NEW: Allow cache to be passed without erroring ---
    backtest_results_cache: Optional[Any] = None

    # --- FIX: Validator to handle JSON strings ---
    @field_validator('strategy_params', 'visual_strategy_json', 'backtest_results_cache', mode='before')
    @classmethod
    def parse_json_strings(cls, v: Any) -> Any: # Return type Any
        if isinstance(v, str):
...
async def update_bot(
    ...
    Fixes transaction handling to prevent 500 Internal Server Errors.
    """
    # 1. Fetch the bot first
    bot = await db.get(TradingBot, bot_id)
    ...
    # 2. Get update data, filtering out None values
    ...
    try:
        for key, value in update_data.items():
            if value is not None:
                # Handle JSON fields: The DB stores them as Text, but Pydantic might pass them as Dicts.
                if key in ["strategy_params", "visual_strategy_json", "backtest_results_cache"]:
        ...
        logger.error(f"Error updating bot {bot_id}: {e}", exc_info=True)
...
# --- Add this new endpoint to the users_router ---
@users_router.get("/me/status", response_model=UserStatusResponse)
async def get_user_status(current_user: User = Depends(get_current_user)):
    """
    A lightweight endpoint for clients to quickly check the current user's
    role and subscription status without fetching the entire user object.
    """
    return UserStatusResponse(
        is_superuser=(current_user.role == UserRole.SUPERUSER.value),
...
async def connect_to_mt5(creds: MT5CredentialsCreate, user: User = Depends(get_current_user)):
    """
    Initiates a connection to the user's MT5 account and returns account details.
    """
    ...
    # 2. Fetch Account Summary (Balance, Equity, etc.)
    details = await mt5_gateway_service.get_account_summary()

    # 3. Return combined response
    return {
        "status": "success", 
        "message": "MT5 Gateway connected successfully.",
        "details": details # This sends the balance to the frontend
    }
...
async def setup_two_factor_auth(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    # --- FIX: Use the correctly imported SVG factory ---
...
# --- THIS IS THE FIX ---
# We add a second decorator to the existing login function.
# The first one, `/token`, is used by the web app's OAuth2 flow.
# The new one, `/login`, is what the desktop app is calling.

#@auth_router.post("/token", response_model=Token, include_in_schema=False) # Hide from docs to avoid confusion
@auth_router.post("/login", response_model=Token) # This is the new endpoint
async def unified_login(request: UnifiedLoginRequest, db: AsyncSession = Depends(get_db)):
    """
    A unified login endpoint that handles both Firebase ID tokens (from web)
    and one-time activation tokens (from desktop serial key validation).
    """
    user_id = None
    
    if request.id_token:
        # --- Firebase Web Login Flow ---
        try:
    ...
    elif request.activation_token:
        # --- Desktop App Activation Flow ---
        try:
            payload = jwt.decode(request.activation_token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM])
            if payload.get("scope") != "activation":
                raise HTTPException(status_code=401, detail="Invalid token scope for activation.")
            user_id = payload.get("sub")
        except JWTError:
            raise HTTPException(status_code=401, detail="Invalid or expired activation token.")
    
    else:
        raise HTTPException(status_code=400, detail="Either id_token or activation_token must be provided.")

    if not user_id:
        raise HTTPException(status_code=401, detail="Could not authenticate user from the provided token.")

    user = await db.get(User, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User account not found in the database.")

    # Generate final, long-lived access and refresh tokens
    access_token = create_access_token(data={"sub": user.id, "ver": user.token_version, "role": user.role})
    refresh_token = create_refresh_token(data={"sub": user.id})
...
# ... in the auth_router section ...

@auth_router.post("/desktop-login", response_model=Token)
async def desktop_login(
    login_data: DesktopLoginRequest, 
    db: AsyncSession = Depends(get_db)
):
    """
    A dedicated, secure endpoint for the PyQt6 desktop application to log in
    using an email and password.
    """
    # Find the user by email
    user = await db.scalar(select(User).where(User.email == login_data.username))

    if not user:
        raise HTTPException(status_code=401, detail="Incorrect username or password.")

    # --- IMPORTANT: Verify the password with Firebase Authentication ---
    # This is the most secure way. We do not store passwords ourselves.
    # We will try to sign the user in with Firebase to verify their password.
    # This requires a custom function, as Firebase Admin SDK doesn't have a direct "checkPassword".
    
    # For this implementation, we will use a simplified check for the superuser
    # and assume regular users are handled via web login.
    
    is_password_valid = False
    if user.role == UserRole.SUPERUSER.value and user.email == settings.SUPERUSER_EMAIL:
        # For the superuser, we check against the hashed password
        is_password_valid = pwd_context.verify(login_data.password, SUPERUSER_PASSWORD_HASH)
    else:
        # For regular users, a desktop email/password login is complex and less secure
        # than token-based flows. For now, we'll disallow it for non-superusers.
        raise HTTPException(status_code=403, detail="Desktop login is currently only available for superuser accounts.")
...
async def generate_serial_number(
    ...
    """
    Generates a new, unique, and complete serial number.
    """
    # ... (user validation logic is the same)
    user_to_assign = await db.get(User, request.user_id)
    if not user_to_assign:
        raise HTTPException(status_code=404, detail=f"User with ID {request.user_id} not found.")

    # --- THIS IS THE ROBUST FIX FOR KEY GENERATION ---
    
    # 1. Define the character set for the key.
    #    (Excluding ambiguous characters like O, 0, I, 1 can be a good practice)
    ALPHANUMERIC_CHARS = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'
    
    # 2. Generate the four segments of the key.
    segments = [''.join(secrets.choice(ALPHANUMERIC_CHARS) for _ in range(4)) for _ in range(4)]
    
    # 3. Join the segments into the final key format.
    new_key = f"QETD-{'-'.join(segments)}"

    # ----------------------------------------------------
    
    # Check for the extremely rare case of a collision, and regenerate if necessary.
    # In a high-volume system, a while loop would be used here.
    existing_key = await db.scalar(select(SerialNumber).where(SerialNumber.serial_key == new_key))
    if existing_key:
        # This is astronomically unlikely but is a robust check.
        raise HTTPException(status_code=500, detail="Serial key collision occurred. Please try again.")

    serial = SerialNumber(
        serial_key=new_key,
        assigned_user_id=request.user_id,
        associated_plan=request.plan.value
    )
    db.add(serial)
    await db.commit()
    await db.refresh(serial)
    
    logger.info(f"Generated new serial key {serial.id} ({serial.serial_key}) for user {user_to_assign.email}.")
    return serial
...
@superuser_router.delete("/serials/{serial_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_serial_number(
        serial_id: int,
        db: AsyncSession = Depends(get_db)
):
    """
    Deletes a serial number from the database.
    This can be used to revoke access for a user.
    """
    serial_to_delete = await db.get(SerialNumber, serial_id)

    if not serial_to_delete:
        raise HTTPException(status_code=404, detail="Serial number not found.")

    await db.delete(serial_to_delete)
    await db.commit()

    logger.info(f"Superuser deleted serial key ID: {serial_id}")
    return
