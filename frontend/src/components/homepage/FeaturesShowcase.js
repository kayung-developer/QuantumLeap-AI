...
const FeatureCard = ({ feature, index }) => {
    ...
    return (
        <motion.div ref={ref} variants={variants} initial="hidden" animate={inView ? "visible" : "hidden"} className="p-6 bg-light-secondary dark:bg-secondary border border-light-border dark:border-border-color rounded-lg text-left">
            <div className="text-3xl text-accent mb-4">{feature.icon}</div>
            <h3 className="text-xl font-bold text-light-heading dark:text-white mb-2">{feature.title}</h3>
            <p className="text-light-muted dark:text-light-gray">{feature.description}</p>
        </motion.div>
    );
};
