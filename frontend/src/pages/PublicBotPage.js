...
import { getPublicBotPerformance } from '../api/apiService'; // Add to apiService.js
import Spinner from '../components/common/Spinner';
import Alert from '../components/common/Alert';
import Card from '../components/common/Card';
import Button from '../components/common/Button';
import { FaChartLine, FaHistory, FaRobot, FaArrowRight } from 'react-icons/fa';
            ...
            <main className="container mx-auto p-4 md:p-8">
                ...
                {perf && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
                        <Stat label="Backtest Return" value={`${perf.total_return_pct.toFixed(1)}%`} valueColor={perf.total_return_pct > 0 ? 'text-success' : 'text-danger'} />
                        <Stat label="Sharpe Ratio" value={perf.sharpe_ratio.toFixed(2)} />
                        <Stat label="Max Drawdown" value={`${perf.max_drawdown_pct.toFixed(1)}%`} valueColor="text-warning" />
